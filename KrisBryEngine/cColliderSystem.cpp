...
bool cColliderSystem::testForCollisionWithStaticObject(cEntity* entity) {
	...
	cCollider* collider = entity->getComponent<cCollider>();

	//for each of the colliders do a test

	//debug rendering
	cTransformComponent* transformComponentSphere = this->pColliderSphere->getComponent<cTransformComponent>();
	cRenderMeshComponent* meshComponentSphere = this->pColliderSphere->getComponent<cRenderMeshComponent>();
	...
	for (cCollider::sPhysicsObject& x : collider->physics_objects) {
		...
		if (aabbIt != this->staticAABBTriangles->m_mapAABBs.end()) {
			cTransformComponent* transformComponentCube = this->pAABBCube->getComponent<cTransformComponent>();
			cRenderMeshComponent* meshComponentCube = this->pAABBCube->getComponent<cRenderMeshComponent>();

			//highlight the aabb we are testing
			transformComponentCube->scale = glm::vec3(AABB_HEIRARCHY_HALF_LENGTH);
			meshComponentCube->materialDiffuse = glm::vec4(1.0f, 0.0f, 0.0f, 1.0f);
			transformComponentCube->setPosition((*aabbIt).second->center);
			meshComponentCube->bIsVisible = true;

			glm::mat4 matIdentity(1.0f);
			cMeshRenderSystem::getInstance()->drawObject(this->pAABBCube, matIdentity);
			...
			// *************************************** Start of Physics Final Question 2
			int closestTriangle = 0;
			float minDistance = 1000.0f;

			sAABB_Triangle& testTriangle = trisToTest->at(0);
			glm::vec3 collisionPoint = cPhysics::closestPtPointTriangle(transformComponent->getPosition(), testTriangle.verts[0], testTriangle.verts[1], testTriangle.verts[2]);

			for(unsigned int i = 0; i != trisToTest->size(); ++i) {
				...
				// Calculate the closest point between the collision object and the current triangle
				glm::vec3 closestPoint = cPhysics::closestPtPointTriangle(transformComponent->getPosition(), trisToTest->at(i).verts[0],
					trisToTest->at(i).verts[1], trisToTest->at(i).verts[2]);
				float distance = glm::abs(glm::distance(transformComponent->getPosition(), closestPoint));
			...
			// Respond to the collision by offsetting the y getPosition()
			glm::vec3 pos = transformComponent->getPosition();
			pos.y = collisionPoint.y + 2.5f;
			transformComponent->setPosition(pos);

			if (entity->entityType != 0) {
				//draw the sphere green/red (red == collision)
				transformComponentSphere->scale = glm::vec3(((sSphere*)x.theShape)->radius);
				meshComponentSphere->materialDiffuse = hitTriangle ? glm::vec4(1.0f, 0.0f, 0.0f, 1.0f) : glm::vec4(1.0f, 1.0f, 0.0f, 1.0f);
				transformComponentSphere->getPosition() = contactPoint_WorldSpace;
				meshComponentSphere->bIsVisible = true;
				matIdentity = glm::mat4(1.0f);
				cMeshRenderSystem::getInstance()->drawObject(this->pColliderSphere, matIdentity);
			...
			transformComponentSphere->scale = glm::vec3(((sSphere*)x.theShape)->radius);
			meshComponentSphere->materialDiffuse = glm::vec4(0.0f, 1.0f, 0.0f, 1.0f);
			transformComponentSphere->setPosition(contactPoint_WorldSpace);
			meshComponentSphere->bIsVisible = true;
			glm::mat4 matIdentity(1.0f);
			cMeshRenderSystem::getInstance()->drawObject(this->pColliderSphere, matIdentity);
			meshComponentSphere->bIsVisible = false;
...
void cColliderSystem::drawColliders(cEntity* entity) {
	...
	{
		cCollider* collider = pControlledEntity->getComponent<cCollider>();
		cTransformComponent* transformComponent = pControlledEntity->getComponent<cTransformComponent>();
		cRenderMeshComponent* meshComponent = pControlledEntity->getComponent<cRenderMeshComponent>();
		...
		for (cCollider::sPhysicsObject x : collider->physics_objects) {
			...
			glm::vec3 oldPosition = transformComponentSphere->getPosition();
