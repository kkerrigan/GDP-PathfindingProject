...
int wait(lua_State* luaState) {
	
	const char* name = lua_tostring(luaState, 1);
	int duration = lua_tonumber(luaState, 2);

	cWait* orientCommand = new cWait(duration);
	std::cout << "wait command created on " << name << std::endl;
...
int orientCameraToEntity(lua_State* luaState) {
	
	const char* name = lua_tostring(luaState, 1);
	int lookAtEntity = lua_tointeger(luaState, 2);
	float speed = lua_tonumber(luaState, 3);
	float easeIn = lua_tonumber(luaState, 4);
	float easeOut = lua_tonumber(luaState, 5);

	cFlyCamera* camera = &cSceneManager::getInstance()->getActiveScene()->flyCamera;
	cEntity* entityB = cSceneManager::getInstance()->getActiveScene()->findObjectById(lookAtEntity);

	cCameraOrientToEntity* orientCommand = new cCameraOrientToEntity(camera, entityB, speed, easeIn, easeOut);
	std::cout << "orientCommand for camera created." << std::endl;
...
int fireCannons(lua_State* luaState) {

	const char* name = lua_tostring(luaState, 1);
	int attackingShip = lua_tointeger(luaState, 2);
	int targetShip = lua_tointeger(luaState, 3);

	cEntity* pAttackingShip = cSceneManager::getInstance()->getActiveScene()->findObjectById(attackingShip);
	cEntity* pTargetShip = cSceneManager::getInstance()->getActiveScene()->findObjectById(targetShip);

	cFireCannons* fireCannonsCommand = new cFireCannons(pAttackingShip, pTargetShip);
	std::cout << "fireCannons for entity " << attackingShip << " created." << std::endl;

	commandGroups[name]->commands.push_back(fireCannonsCommand);
