...
cFollowCurve::cFollowCurve(cEntity* entity, glm::vec3 destination, float time, glm::vec3 controlPoint, float easeIn, float easeOut) {
	...
	this->startPosition = this->entity->getComponent<cTransformComponent>()->getPosition();
	this->lastPosition = this->entity->getComponent<cTransformComponent>()->getPosition();
...
void cFollowCurve::update(float deltaTime) {
	...
	if (!this->hasStarted) {
		...
		this->startPosition = transformComponent->getPosition();
	...
	if (transformComponent->getPosition() != this->lastPosition) {
		this->traveledDistance += glm::distance(transformComponent->getPosition(), this->lastPosition);
	}

	// Update the previous position
	this->lastPosition = transformComponent->getPosition();
	...
	glm::vec3 direction = glm::normalize(glm::vec3(x, y, z) - transformComponent->getPosition());

	if(glm::distance(glm::vec3(0.0f), direction) > 0.5f) {
		
		float distance = glm::distance(this->finalPosition, this->controlPoint);

		glm::vec3 direction = glm::normalize(this->finalPosition - this->controlPoint);
		
		transformComponent->orientation = glm::toMat4(glm::quatLookAt(-direction, UP));
	}
	...
	if (transformComponent->getPosition() == this->finalPosition || this->elapsedTime >= this->duration) {
		this->finished = true;
	}
