...
cFreeTypeRenderer::cFreeTypeRenderer(){
	...
	this->sx = 2.0f / cEngine::getInstance()->getConfigSettings()->width;
	this->sy = 2.0f / cEngine::getInstance()->getConfigSettings()->height;
	this->yoffset = 30.0f;
	this->xoffset = 8 * sx;
...
void cFreeTypeRenderer::renderText(const char *text, float x, float y, float sx, float sy, glm::vec4 color) {
	...
	cShaderManager::cShaderProgram* pSP = cShaderManager::getInstance()->pGetShaderProgramFromFriendlyName("FreeTypeShader");
	...
	glUniform4f(pSP->getUniformID_From_Name("color"), color.r, color.g, color.b, color.a);
	...
	glUniform1i(pSP->getUniformID_From_Name("tex"), 10);
	...
	glEnableVertexAttribArray(pSP->getUniformID_From_Name("coord"));
	glBindBuffer(GL_ARRAY_BUFFER, this->dp_vbo);
	glVertexAttribPointer(pSP->getUniformID_From_Name("coord"), 4, GL_FLOAT, GL_FALSE, 0, 0);
	...
	glDisableVertexAttribArray(pSP->getUniformID_From_Name("coord"));
